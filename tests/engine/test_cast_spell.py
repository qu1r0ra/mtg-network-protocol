...
def _two_player_state(*, hand: list[str] | None = None) -> GameState:
    ...
    state.players = {
        "alice": PlayerState(
            player_id="alice",
            life=20,
            hand=hand if hand is not None else ["lightning_bolt_001"],
        ),
...
def test_cast_spell_wrong_target_count_rejected():
    ...
    outbounds = cast.handle_cast_spell(
        state, "player_1", _bolt_pdu(targets=["bob", "alice"])
    )
...
def test_cast_spell_illegal_target_rejected():
    ...
    outbounds = cast.handle_cast_spell(
        state, "player_1", _bolt_pdu(targets=["nonexistent_creature"])
    )
...
def test_cast_spell_targeting_a_permanent_queues_a_targeted_trigger(monkeypatch):
    ...
    state.players["bob"].battlefield.append(
        Permanent(id="phantasmal_bear_001", power=2, toughness=2)
    )

    cast.handle_cast_spell(
        state, "player_1", _bolt_pdu(targets=["phantasmal_bear_001"])
    )
...
def test_cast_spell_end_to_end_lightning_bolt_resolves_through_engine(make_engine):
    ...
    engine.handle(
        "player_1",
        PriorityPass(seq_num=token_after_cast).model_dump_json().encode("utf-8"),
    )
    token_for_bob = engine.state.priority_token
    outbounds = engine.handle(
        "player_2",
        PriorityPass(seq_num=token_for_bob).model_dump_json().encode("utf-8"),
    )
...
def test_cast_kicked_vines_of_vastwood_end_to_end_protects_and_pumps_through_engine(
    make_engine,
):
    ...
    engine.state.players = {
        "alice": PlayerState(
            player_id="alice",
            life=20,
            hand=["vines_of_vastwood_001", "lightning_bolt_001"],
        ),
        "bob": PlayerState(
            player_id="bob",
            life=20,
    ...
    engine.handle(
        "player_1",
        PriorityPass(seq_num=engine.state.priority_token)
        .model_dump_json()
        .encode("utf-8"),
    )
