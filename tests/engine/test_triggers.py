...
def test_gray_merchant_end_to_end_resolves_etb_trigger_through_engine(make_engine):
    ...
    cast_pdu = CastSpell(
        seq_num=1,
        card_id="gray_merchant_001",
        targets=[],
        mana_payment={"B": 2, "generic": 3},
    )
    ...
    engine.handle(
        "player_1", PriorityPass(seq_num=token).model_dump_json().encode("utf-8")
    )
    token = engine.state.priority_token
    engine.handle(
        "player_2", PriorityPass(seq_num=token).model_dump_json().encode("utf-8")
    )
    ...
    assert any(
        o.pdu.type == "STACK_RESOLVE" and o.pdu.result == "RESOLVED" for o in outbounds
    )
...
def test_gravedigger_end_to_end_resolves_targeted_trigger_through_engine(make_engine):
    ...
    engine.state.players = {
        "alice": PlayerState(
            player_id="alice",
            life=20,
            hand=["gravedigger_001"],
            graveyard=["gray_merchant_001"],
        ),
        "bob": PlayerState(player_id="bob", life=20),
    }
    engine.state.active_player = "alice"
...
def test_goblin_bushwhacker_end_to_end_kicked_pumps_creatures_through_engine(
    make_engine,
):
    ...
    engine.state.players = {
        "alice": PlayerState(
            player_id="alice", life=20, hand=["goblin_bushwhacker_001"]
        ),
        "bob": PlayerState(player_id="bob", life=20),
    }
    engine.state.active_player = "alice"
    ...
    cast_pdu = CastSpell(
        seq_num=1,
        card_id="goblin_bushwhacker_001",
        targets=[],
        mana_payment={"R": 2, "generic": 1},
        kicked=True,
    )
    engine.handle("player_1", cast_pdu.model_dump_json().encode("utf-8"))
...
def test_goblin_bushwhacker_end_to_end_unkicked_discards_trigger_through_engine(
    make_engine,
):
...
def test_goblin_guide_end_to_end_attack_trigger_reveals_land_through_engine(
    make_engine,
):
    ...
    engine.state.players = {
        "alice": PlayerState(
            ...
            battlefield=[
                Permanent(
                    id="goblin_guide_001", power=2, toughness=2, damage=0, haste=True
                )
            ],
        ),
        "bob": PlayerState(
            player_id="bob", life=20, library=["mountain_001", "grizzly_bears_002"]
        ),
    }
    engine.state.active_player = "alice"
    engine.state.priority_holder = "alice"
    ...
    declare_pdu = DeclareAttackers(
        seq_num=1,
        attackers=[AttackerDeclaration(creature_id="goblin_guide_001", target="bob")],
    )
    engine.handle("player_1", declare_pdu.model_dump_json().encode("utf-8"))

    assert (
        engine.state.pending_attack_trigger == []
    )  # drained by the trailing priority.grant
    assert len(engine.state.stack) == 1
    assert engine.state.stack[0].item_type == "TRIGGER_ABILITY"
...
def test_monastery_swiftspear_end_to_end_prowess_cast_trigger_through_engine(
    make_engine,
):
    ...
    engine.state.players = {
        "alice": PlayerState(
            ...
            battlefield=[
                Permanent(id="monastery_swiftspear_001", power=1, toughness=2)
            ],
        ),
        "bob": PlayerState(player_id="bob", life=20),
    }
    ...
    cast_pdu = CastSpell(
        seq_num=1, card_id="lightning_bolt_001", targets=["bob"], mana_payment={"R": 1}
    )
    engine.handle("player_1", cast_pdu.model_dump_json().encode("utf-8"))

    assert (
        engine.state.pending_cast_trigger == []
    )  # drained by the trailing priority.grant
...
def test_phantasmal_bear_end_to_end_sacrifices_when_targeted_through_engine(
    make_engine,
):
    ...
    assert (
        engine.state.pending_targeted_trigger == []
    )  # drained by the trailing priority.grant
    assert len(engine.state.stack) == 2
    assert engine.state.stack[0].item_type == "SPELL"
    assert engine.state.stack[1].item_type == "TRIGGER_ABILITY"
